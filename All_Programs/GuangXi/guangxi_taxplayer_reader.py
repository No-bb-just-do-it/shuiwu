...
class GuangXiTaxplayerReader(TaxplayerReader):
    def __init__(self):
        super(GuangXiTaxplayerReader, self).__init__()
        self.province = u'广西壮族自治区'
        self.set_config()
        # self.today = "'%2017-12-25%'"
                ...
                for m in range(count):
                    ...
                    if match_fields:
                        ...
                        if 'qsje' not in keys and not cqsje_condition:
                            self.log(str(num_info + 1) + '  qsje not in keys and not cqsje_condition  ' + str(info[num_info][4].encode('utf-8')))
                            continue
                        self.log(str(num_info + 1) + '  qsje not in keys and not cqsje_condition  ' + str(info[num_info][4].encode('utf-8')))
                        start_idx = self.get_excel_start_idx(table, rows)
